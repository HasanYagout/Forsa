<x-app-layout>
    @section('title', 'Tender View')
    <section class="w-full flex justify-center my-10 px-4 sm:px-6">
        <section id="tender-listings" class="w-full lg:w-[60%] mx-4 sm:mx-6 flex flex-col items-center gap-4">
            <section class="w-full relative bg-white transition rounded-xl shadow-sm flex flex-col items-center gap-4 mb-4 border border-gray-200">
                <!-- Tender Header -->
                <section class="border-b flex flex-col flex-shrink-0 px-6 py-10 sm:flex-row w-full">
                    <!-- Company Logo -->
                    ...
                    @endphp
                        ...
                        <div class="absolute top-2 {{ $isRtl ? 'left-2 sm:left-4' : 'right-2 sm:right-4' }} z-10">
                            <button class="bookmark-btn cursor-pointer flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white shadow-sm border border-gray-200
                                 {{ auth()->check() ? '' : 'guest-bookmark' }}"
                                    data-id="{{ $tender->id }}"
                                    data-type="{{ auth()->check() ? auth()->user()->getBookmarkType($tender) : '' }}"
                                    data-bookmarked="{{ auth()->check() && auth()->user()->hasBookmarked($tender) ? 'true' : 'false' }}">
                                <svg xmlns="http://www.w3.org/2000/svg"
                                     fill="{{ auth()->check() && auth()->user()->hasBookmarked($tender) ? 'currentColor' : 'none' }}"
                                     viewBox="0 0 24 24"
                                     stroke-width="1.5"
                                     stroke="currentColor"
                        ...
                        <section class="flex flex-col justify-between w-full ms-0 sm:ms-6">
                            <!-- Tender Title -->
                            <h1 class="font-semibold text-lg mb-2">{{ $tender->title }}</h1>
                            ...
                            <section class="flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center text-gray-500 text-xs sm:text-sm gap-1 sm:gap-2 mb-3">
                                ...
                                <a target="_blank" href="{{ route('tenders.index', ['company' => $tender->company->id]) }}"
                                   class="font-semibold flex items-center gap-1 hover:underline hover:text-primary">
                                    ...
                                    <span class="truncate max-w-[120px] sm:max-w-none">{{ $tender->company->name }}</span>
                                </a>
                            </span>
                                ...
                                <!-- Location -->
                                @php
                                    $cities = is_array($tender->location) ? $tender->location : (array)$tender->location;
                                ...
                                @else
                                    @foreach($cities as $city)
                                        <a href="{{ route('tenders.index', ['location' => $city]) }}" class="text-sm font-bold text-black hover:text-primary hover:underline whitespace-nowrap">
                                            {{ $city }}
                                        </a>
                                        @if(!$loop->last)
                                ...
                                <div class="text-green-600 font-bold flex items-center gap-1">
                                    ...
                                    <span>{{__('Published')}}: {{ $tender->created_at->format('j M Y')}}</span>
                                </div>
                                ...
                                <div class="text-red-600 font-bold flex items-center gap-1">
                                    ...
                                    <span>{{__('Deadline')}}: {{ $tender->deadline->format('j M Y')}}</span>
                                </div>
                            </section>
                        </section>
                    ...
                    <section class="flex gap-2 sm:gap-3 mt-3 sm:mt-0 {{ app()->getLocale() === 'ar' ? 'justify-start' : 'justify-end' }} w-full sm:w-auto">
                        <div class="flex items-center space-x-2 sm:space-x-3">
                            <!-- Facebook -->
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('tenders.view', ['slug' => $tender->slug])) }}"
                               target="_blank"
                            ...
                            <!-- Twitter -->
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(route('tenders.view', ['slug' => $tender->slug])) }}"
                               target="_blank"
                            ...
                            <!-- LinkedIn -->
                            <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode(route('tenders.view', ['slug' => $tender->slug])) }}"
                               target="_blank"
                ...
                <section class="text-white w-full text-xs mt-3">
                    <h1 class="p-2 text-xl w-full flex bg-gray-300">{{__('Tender Description')}}</h1>
        ...
        <section id="sidebar-section"
            ...
            @if(now()->lte($tender->deadline))
                <section class="bg-white w-full shadow-sm border border-gray-200 rounded-lg p-4">
                    <a href="{{$tender->link}}" target="_blank" class="bg-secondary text-center cursor-pointer text-white block px-4 py-2 rounded-lg w-full">{{__('Apply Now')}}</a>
                </section>
            @endif

            <!-- Similar Tenders -->
            <section class="bg-white w-full shadow-sm border border-blue-200 rounded-lg p-4 space-y-4">
                <h1 class="text-lg font-semibold">{{__('Similar Tenders')}}</h1>
                @if(count($similar_tenders) > 0)
                    @foreach($similar_tenders as $similarTender)
                        <a href="{{route('tenders.view',['slug'=>$similarTender->slug])}}" class="border-[#e3e3e0] border-b flex items-center pb-3 space-x-4">
                            <div class="flex-shrink-0">
                                <img src="{{asset('storage').'/'. $similarTender->company->logo }}" alt="{{ $similarTender->company_name }} Logo" class="w-12 h-12 rounded-full border-gray-300 border object-cover"/>
                            </div>
                            <div>
                                <h1 class="text-sm text-gray-900">{{ $similarTender->title }}</h1>
                            </div>
                        </a>
                    @endforeach
                @else
                    <h1 class="text-center">{{__('No Similar Tenders')}}</h1>
                @endif
            </section>
...
{{--                            <div class="flex-shrink-0">--}}
{{--                                <img src="{{asset('storage').'/'. $similarJob->company->logo }}" alt="{{ $similarJob->company_name }} Logo" class="w-12 h-12 rounded-full border-gray-300 border object-cover"/>--}}
{{--                            </div>--}}
{{--                            <div>--}}
